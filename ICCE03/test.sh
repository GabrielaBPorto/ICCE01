...
plot() {
    ...
    for n in ${ns[@]}; do
      echo -n "$n " >> tmp
        mvv=$(cat $n.$group.csv | grep "$2" | cut -d\| -f 3 | tr '\n' '\t' | cut -f 1 | sed -e 's/ //g')
        mmv=$(cat $n.$group.csv | grep "$2" | cut -d\| -f 3 | tr '\n' '\t' | cut -f 2 | sed -e 's/ //g')
        echo -n "$mvv $mmv " >> tmp
      echo "" >> tmp
    done
    gnuplot $group.data
...
#     mvv=$(cat $n.FLOPS_AVX.csv | grep "$2" | cut -d\| -f 3 | tr '\n' '\t' | cut -f 1 | sed -e 's/ //g')
#     mmv=$(cat $n.FLOPS_AVX.csv | grep "$2" | cut -d\| -f 3 | tr '\n' '\t' | cut -f 2 | sed -e 's/ //g')
...
for n in ${ns[@]}; do
  ./geraSL $n >>  $diretorio/sistema_$n.res
  for group in ${groups[@]}; do
    likwid-perfctr -C 3 -g $group -o $diretorio/$n.$group.csv -f -m ./gaussJacobi-likwid $diretorio/sistema_$n.res
  done
done
